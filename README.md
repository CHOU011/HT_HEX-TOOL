...
Chrome 或 Edge（Firefox / Safari 不支持）



通过 http(s):// 或 localhost 打开 —— 不能直接双击 index.html（file:// 下 Web Serial 被禁用）



首次「打开串口」时浏览器弹窗，需手动选择并授权对应串口
...
固定 "serial-command-table"，工具据此识别
...
serial.dataBits/parity/stopBits
...
将本目录 index.html 推到仓库根目录（或 docs/index.html）
